...
{ 
	GaussianBlur(src, dst, Size(0,0), sigma, sigma); //直接调用opencv函数
}
...
void DrawKeyPointsLowRight(Mat &src, Vector<Keypoint>& keypoints, CvSize size)
{
	...
	for(int i = 0; i < keypoints.size(); i++)
	{
		...
		CvScalar color = {255, 0 ,0};
		circle(src, Point(keypoints[i].dx + size.width, keypoints[i].dy + size.height), 3, color);
		keypoints[i].dx += size.width;
		keypoints[i].dy += size.height;
		j++;
	}
}
